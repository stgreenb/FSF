def _convert_feature(feature_data, item_type):
    ...
    return {
        ...
        "system": {
            "description": {
                "value": feature_data.get("description"),
                "director": ""
            }
        }
    }
...
def _convert_feature(feature_data, item_type, compendium_items):
    ...
    name = TextNormalizer.normalize_text(original_name) if original_name else original_name
    ...
    if name in known_mappings and known_mappings[name] in compendium_items:
        ...
        if compendium_type:
            compendium_item = next((item for item in compendium_items.values()
                                   if item.get("name", "").lower() == name.lower()
                                   and item.get("type", "").lower() == compendium_type.lower()), None)
        ...
        if not compendium_item:
            compendium_item = next((item for item in compendium_items.values() if item.get("name", "").lower() == name.lower()), None)

        # Also try with quotes (some names have quotes in compendium)
        if not compendium_item:
            quoted_name = f'"{name}"'
            compendium_item = next((item for item in compendium_items.values() if item.get("name", "").lower() == quoted_name.lower()), None)
    ...
    from converter.description_transfer import DescriptionTransfer
    description = DescriptionTransfer.transfer_description(feature_data, compendium_item)
    ...
    if item_type == "ability" and "data" in feature_data and "ability" in feature_data["data"]:
        ability_data = feature_data["data"]["ability"]
        source_action_type = ability_data.get("type", {}).get("usage", "main")
        result_item["system"].update({
            ...
            "effect": {
                "before": ability_data.get("description", ""),
                "after": ""
            },
            "power": {
                "roll": {
...
def convert_character(character_data, compendium_items, strict=False, verbose=False):
    ...
    foundry_character = {
        ...
        "system": {
            "stamina": {
                "value": base_stamina - character_data.get("state", {}).get("staminaDamage", 0),
    ...
    print(f"Calculated Movement Speed: {movement_speed} (ancestry {ancestry_speed or 'base 5'} + kit bonus {kit_speed_bonus})")
    ...
    if "characteristics" in class_data:
        ...
        if class_data.get("featuresByLevel"):
            character_level = class_data.get("level", 1)  # Default to level 1 if not specified
                ...
                if level_num <= character_level:
                    for feature in level_data.get("features", []):
                        if feature.get("type") == "Characteristic Bonus":
                            char_name = feature.get("data", {}).get("characteristic", "").lower()
            ...
            if feature.get("type") == "Choice":
                ...
                if item:
                    ...
                    if "system" in item and "advancements" in item["system"]:
                        for advancement_id, advancement in item["system"]["advancements"].items():
                            if advancement.get("type") == "itemGrant" and "pool" in advancement:
                                # Add abilities from the ancestry trait's advancement pool
                                for pool_item in advancement["pool"]:
                                    if "uuid" in pool_item:
                                        # Look up the ability in compendium by UUID
                                        for comp_item in compendium_items.values():
        ...
        for level_data in hero_class.get("featuresByLevel", []):
            ...
            if level_num <= character_level:
                for feature in level_data.get("features", []):
                    ...
                    # Always skip Skill Choice features - they're handled separately by _process_skills_from_advancements
                    ...
                    # Skip unselected domain features (but process Perk and Project selected items)
                    ...
                    skip_patterns = ["pt Ability", "Signature Ability", "Kit", "1st-Level", "4th-Level", "5th-Level", "7th-Level", "9th-Level"]
                    ...
                    if feature_type in ["Perk", "Project"]:
                        ...
                        for selected_item in selected_items:
                            item = _convert_feature(selected_item, feature_type.lower(), compendium_items)
                            if item: foundry_character["items"].append(item)
                        continue
                    ...
                    if feature.get("type") == "Choice":
                        ...
                        for nested_feature in nested_features:
                            if nested_feature.get("type") == "Ability":
                                ...
                                ability_data = nested_feature.get("data", {}).get("ability", nested_feature)
                                ...
                                item = _convert_feature(reconstructed_feature, "ability", compendium_items)
                                if item: foundry_character["items"].append(item)
                        continue
                    elif feature_type not in ["Bonus", "Characteristic Bonus", "Heroic Resource Gain"]:
                        ...
                        item_type = "ability" if feature_type == "Ability" else "feature"

                        # For Ability type, extract the nested ability data
                        if feature_type == "Ability" and "data" in feature and "ability" in feature["data"]:
                            feature_to_convert = feature["data"]["ability"]
                        else:
                            feature_to_convert = feature

                        item = _convert_feature(feature_to_convert, item_type, compendium_items)
                        if item: foundry_character["items"].append(item)
    ...
    if career:
        ...
        for feature in career.get("features", []):
            ...
            if processed_feature: foundry_character["items"].append(processed_feature)
        ...
        for level_data in subclass.get("featuresByLevel", []):
            ...
            if level_num <= character_level:
                for feature in level_data.get("features", []):
                    ...
                    if feature_type not in ["Bonus", "Characteristic Bonus", "Heroic Resource Gain"]:
                        # Use the feature's actual type for conversion
                        item_type = "ability" if feature_type == "Ability" else "feature"
    ...
    for level_data in class_data.get("featuresByLevel", []):
        ...
        if level_num <= character_level:
            for feature in level_data.get("features", []):
                ...
                if feature.get("type") == "Class Ability" and "selectedIDs" in feature.get("data", {}):
                ...
                elif feature.get("type") == "Kit" and "selected" in feature.get("data", {}):
    ...
    for kit in selected_kits:
        ...
        if kit_item:
            ...
            if "system" in kit_item and "advancements" in kit_item["system"]:
                for advancement_id, advancement in kit_item["system"]["advancements"].items():
                    if advancement.get("type") == "itemGrant" and "pool" in advancement:
    ...
    for item in character_data.get("state", {}).get("inventory", []):
        foundry_character["items"].append(_convert_feature(item, "treasure", compendium_items))

    # Post-processing: Extract skills from item advancements
    _populate_advancement_selections(foundry_character, character_data, compendium_items)
...
def _populate_advancement_selections(character_data, source_data, compendium_items):
    ...
    if ancestry:
        all_selected_skills.update(collect_skills_from_features(ancestry.get("features", [])))
        ...
        for section_name in ["language", "environment", "organization", "upbringing"]:
            if section_name in culture:
                ...
                if section.get("type") == "Skill Choice":
                    ...
                    if skills:
                        all_selected_skills.update([_normalize_skill_name(s) for s in skills])
    
    # From career
    career = source_data.get("career", {})
    if career:
        all_selected_skills.update(collect_skills_from_features(career.get("features", [])))
    
    # From class
    class_data = source_data.get("class", {})
    if class_data:
        for level_data in class_data.get("featuresByLevel", []):
            all_selected_skills.update(collect_skills_from_features(level_data.get("features", [])))
    ...
    for item in character_data.get("items", []):
        if item.get("type") not in ["ancestry", "culture", "career", "class", "subclass"]:
            continue
        ...
        for advancement_id, advancement in item["system"]["advancements"].items():
            ...
            if advancement_type == "skill":
                ...
                if choices:
                    ...
                    if selected_from_choices:
                        ...
                        item["flags"]["draw-steel"]["advancement"][advancement_id] = selection_data
            ...
            # Handle language advancements
                ...
                def collect_languages_from_features(features_list):
                    ...
                    for feature in features_list:
                        if feature.get("type") == "Language Choice":
                            ...
                            if feature_langs:
                                langs.extend([_normalize_skill_name(l) for l in feature_langs])
                    return langs
                ...
                if item_type == "culture":
                        ...
                        for section_name in ["language", "environment", "organization", "upbringing"]:
                            if section_name in culture:
                                ...
                                if section.get("type") == "Language Choice":
                                    ...
                                    if langs:
                                        selected_languages.update([_normalize_skill_name(l) for l in langs])
                
                elif item_type == "career":
                    # Collect only from career
                    career = source_data.get("career", {})
                    if career:
                        selected_languages.update(collect_languages_from_features(career.get("features", [])))
                    ...
                    if class_data:
                        for level_data in class_data.get("featuresByLevel", []):
                            selected_languages.update(collect_languages_from_features(level_data.get("features", [])))
...
def _fix_ability_types(character_data):
    ...
    for item in character_data.get("items", []):
        ...
        if (item.get("type") == "feature" and
            item_system.get("type") == "triggered"):
            item["type"] = "ability"
...
if __name__ == "__main__":
    ...
    print("If you need to debug mapper functionality, modify forgesteel_converter.py instead.")
    import sys
    sys.exit(1)